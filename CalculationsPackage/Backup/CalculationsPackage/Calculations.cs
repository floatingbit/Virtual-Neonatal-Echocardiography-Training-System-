...
namespace CalculationsPackage
{
    class Calculations
    {
        ...
        private void LVMode()
        {
            ...
            this.HR = Math.Abs((60.0 / ((float)this.mouseCoordinates[15].X - (float)this.mouseCoordinates[14].X)) * (float)(this.x_time[0].X - this.x_time[1].X));
            ...
            this.EDV = Math.Pow(this.LVIDd, 3);
            ...
            this.ESV = Math.Pow(this.LVIDs, 3);
        ...
        private void PulmonaryBloodFlow()
        {
            ...
            double avgVel = Math.Abs(MainForm.areaInPixels[0] / ((this.pulmonaryInfo.velocity[0].Y - this.pulmonaryInfo.velocity[1].Y) * 60));

            double TVI = Math.Abs(avgVel * noOfPixelsBetwStartAndEnd / (this.pulmonaryInfo.time[0].X - this.pulmonaryInfo.time[1].X));

            double y_dis = (float)(Math.Sqrt(Math.Pow(this.pulmonaryInfo.diameter[0].Y - this.pulmonaryInfo.diameter[1].Y, 2) + Math.Pow(this.pulmonaryInfo.diameter[0].X - this.pulmonaryInfo.diameter[1].X, 2)));
            double factor = Math.Abs(3 / y_dis);

            this.pulmonaryInfo.CSA = Math.PI * Math.Pow(Math.Abs(this.pulmonaryInfo.diameter[0].Y - this.pulmonaryInfo.diameter[1].Y), 2) / 4 * factor * factor;
    ...
    public class TwoDimInfo
    {
        ...
        public TwoDimInfo()
        {
            this.EDV = this.ESV = /*this.LVSV = this.LVEF = this.LVMd = this.LVMs = */ this.LVMass = this.LVMassPerKg = 0.0;
        }
    }
