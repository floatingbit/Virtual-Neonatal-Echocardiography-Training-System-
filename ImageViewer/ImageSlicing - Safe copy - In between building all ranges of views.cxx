...
public:
	...
	vtkImageInteractionCallback() {
			...
			this->internal_counter_video_loop_limit = 2;
	...
	{
		std::string base_file_name = "C:\\VETS Source Code\\VETS 3D\\Video Frames\\LAX-color\\LAX-color";
		this->video_renderer =	vtkSmartPointer<vtkRenderer>::New();	
		...
		for(int i = 0; i < 35; i++)
		{
			...
			this->imageActor_Par_LAX_Color[i] = vtkSmartPointer<vtkImageActor>::New();
			this->imageActor_Par_LAX_Color[i]->SetInput(jpegReader->GetOutput());
			this->imageActor_Par_LAX_Color[i]->SetVisibility(0);
			this->video_renderer->AddActor(this->imageActor_Par_LAX_Color[i]);
		}

		base_file_name = "C:\\VETS Source Code\\VETS 3D\\Video Frames\\RVOT-color\\RVOT-color";
		...
		for(int i = 0; i < 35; i++)
		{
			...
			this->imageActor_Par_RVOT_Color[i] = vtkSmartPointer<vtkImageActor>::New();
			this->imageActor_Par_RVOT_Color[i]->SetInput(jpegReader->GetOutput());
			this->imageActor_Par_RVOT_Color[i]->SetVisibility(0);
			this->video_renderer->AddActor(this->imageActor_Par_RVOT_Color[i]);
		}

		base_file_name = "C:\\VETS Source Code\\VETS 3D\\Video Frames\\RVOT-Doppler\\RVOT-Doppler";
		...
		for(int i = 0; i < 35; i++)
		{
			...
			this->imageActor_Par_RVOT_Doppler[i] = vtkSmartPointer<vtkImageActor>::New();
			this->imageActor_Par_RVOT_Doppler[i]->SetInput(jpegReader->GetOutput());
			this->imageActor_Par_RVOT_Doppler[i]->SetVisibility(0);
			this->video_renderer->AddActor(this->imageActor_Par_RVOT_Doppler[i]);
		}

		base_file_name = "C:\\VETS Source Code\\VETS 3D\\Video Frames\\SAX-base-color\\SAX-base-color";
		...
		for(int i = 0; i < 35; i++)
		{
			...
			this->imageActor_Par_SAX_Base_Color[i] = vtkSmartPointer<vtkImageActor>::New();
			this->imageActor_Par_SAX_Base_Color[i]->SetInput(jpegReader->GetOutput());
			this->imageActor_Par_SAX_Base_Color[i]->SetVisibility(0);
			this->video_renderer->AddActor(this->imageActor_Par_SAX_Base_Color[i]);
		}

		base_file_name = "C:\\VETS Source Code\\VETS 3D\\Video Frames\\SAX-base-M-mode\\SAX-base-M-mode";
		...
		for(int i = 0; i < 35; i++)
		{
			...
			this->imageActor_Par_SAX_Base_MMode[i] = vtkSmartPointer<vtkImageActor>::New();
			this->imageActor_Par_SAX_Base_MMode[i]->SetInput(jpegReader->GetOutput());
			this->imageActor_Par_SAX_Base_MMode[i]->SetVisibility(0);
			this->video_renderer->AddActor(this->imageActor_Par_SAX_Base_MMode[i]);
		}

		base_file_name = "C:\\VETS Source Code\\VETS 3D\\Video Frames\\Default-M-mode\\Default-M-mode";
		...
		for(int i = 0; i < 35; i++)
		{
			...
			this->imageActor_Par_SAX_Mitral_MMode[i] = vtkSmartPointer<vtkImageActor>::New();
			this->imageActor_Par_SAX_Mitral_MMode[i]->SetInput(jpegReader->GetOutput());
			this->imageActor_Par_SAX_Mitral_MMode[i]->SetVisibility(0);
			this->video_renderer->AddActor(this->imageActor_Par_SAX_Mitral_MMode[i]);
		}

		base_file_name = "C:\\VETS Source Code\\VETS 3D\\Video Frames\\Default-color\\Default-color";
		...
		for(int i = 0; i < 35; i++)
		{
			...
			this->imageActor_Default_Color[i] = vtkSmartPointer<vtkImageActor>::New();
			this->imageActor_Default_Color[i]->SetInput(jpegReader->GetOutput());
			this->imageActor_Default_Color[i]->SetVisibility(0);
			this->video_renderer->AddActor(this->imageActor_Default_Color[i]);
		}
				
		base_file_name = "C:\\VETS Source Code\\VETS 3D\\Video Frames\\Default-Doppler\\Default-Doppler";
		...
		for(int i = 0; i < 35; i++)
		{
			...
			this->imageActor_Default_Doppler[i] = vtkSmartPointer<vtkImageActor>::New();
			this->imageActor_Default_Doppler[i]->SetInput(jpegReader->GetOutput());
			this->imageActor_Default_Doppler[i]->SetVisibility(0);
			this->video_renderer->AddActor(this->imageActor_Default_Doppler[i]);
		}
		...
		for(int i = 0; i < 35; i++)
		{
			...
			this->imageActor_Default_MMode[i] = vtkSmartPointer<vtkImageActor>::New();
			this->imageActor_Default_MMode[i]->SetInput(jpegReader->GetOutput());
			this->imageActor_Default_MMode[i]->SetVisibility(0);
			this->video_renderer->AddActor(this->imageActor_Default_MMode[i]);
		}
	}
	...
	void CheckForSpecificCuts()
	{
		...
		{
		case Parasternal:
			if(roll >= this->videoPlayer->range_Par_LAX_Color->roStart && roll <= this->videoPlayer->range_Par_LAX_Color->roEnd){
				displayText = "Long axis has been detected";
				video_specific_cut_index = Par_LAX;
			}

			else if(roll >= this->videoPlayer->range_Par_RVOT_Color->roStart && roll <= this->videoPlayer->range_Par_RVOT_Color->roEnd){
				displayText = "Short axis has been detected";
				video_specific_cut_index = Par_RVOT;
			}
		...
		if(this->internal_counter_video_loop < this->internal_counter_video_loop_limit)
		{
			...
			if(this->video_specific_cut_index == Par_LAX)
			{
				if(this->video_previous_index > -1)
					imageActor_Par_LAX_Color[this->video_previous_index]->SetVisibility(0);

				imageActor_Par_LAX_Color[this->video_current_index]->SetVisibility(1);
			}

			else 
			{
				if(this->video_previous_index > -1)
					imageActor_Par_RVOT_Color[this->video_previous_index]->SetVisibility(0);

				imageActor_Par_RVOT_Color[this->video_current_index]->SetVisibility(1);
			}
			...
			if(this->video_current_index == 35)
	...
	{					
		...
		if(event == vtkCommand::KeyPressEvent)
		{
			...
			case F2:
				...
				for(int i = 0; i < 35; i++)
				{
					imageActor_Par_LAX_Color[i]->SetVisibility(0);
					imageActor_Par_RVOT_Color[i]->SetVisibility(0);
			...
			if(key == "Insert" || key == "Delete" || key == "Home" || key == "End" || key == "Prior" || key == "Next")
			{
...
public:
	...
	vtkSmartPointer<vtkImageActor> imageActor_Par_LAX_Color[35];
	vtkSmartPointer<vtkImageActor> imageActor_Par_RVOT_Color[35];
	vtkSmartPointer<vtkImageActor> imageActor_Par_RVOT_Doppler[35];
	vtkSmartPointer<vtkImageActor> imageActor_Par_SAX_Base_Color[35];
	vtkSmartPointer<vtkImageActor> imageActor_Par_SAX_Base_MMode[35];
	vtkSmartPointer<vtkImageActor> imageActor_Par_SAX_Mitral_MMode[35];

	vtkSmartPointer<vtkImageActor> imageActor_Api_4_Chamber_Color[35];
	vtkSmartPointer<vtkImageActor> imageActor_Api_4_Chamber_Doppler[35];
	vtkSmartPointer<vtkImageActor> imageActor_Api_5_Chamber_Color[35];
	vtkSmartPointer<vtkImageActor> imageActor_Api_5_Chamber_Doppler[35];

	vtkSmartPointer<vtkImageActor> imageActor_Sub_Coronal_Posterior_Color[35];
	vtkSmartPointer<vtkImageActor> imageActor_Sub_Coronal_Anterior_Color[35];
	vtkSmartPointer<vtkImageActor> imageActor_Sub_Sagital_Bicaval_Color[35];
	vtkSmartPointer<vtkImageActor> imageActor_Sub_Sagital_Bicaval_Doppler[35];
	vtkSmartPointer<vtkImageActor> imageActor_Sub_Transverse_Color[35];
	vtkSmartPointer<vtkImageActor> imageActor_Sub_Longitudanal_Aorta_Color[35];
	vtkSmartPointer<vtkImageActor> imageActor_Sub_Longitudanal_IVC_Color[35];

	vtkSmartPointer<vtkImageActor> imageActor_Sup_Aortic_Arch_Color[35];
	vtkSmartPointer<vtkImageActor> imageActor_Sup_Ductal_Cut_Color[35];
	vtkSmartPointer<vtkImageActor> imageActor_Sup_Crab_View_Color[35];

	vtkSmartPointer<vtkImageActor> imageActor_Default_Color[35];
	vtkSmartPointer<vtkImageActor> imageActor_Default_Doppler[35];
	vtkSmartPointer<vtkImageActor> imageActor_Default_MMode[35];
