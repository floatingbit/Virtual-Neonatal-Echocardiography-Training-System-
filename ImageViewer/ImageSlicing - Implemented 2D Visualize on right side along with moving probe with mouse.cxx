...
public:
	...
	{		
		...
		if (event == vtkCommand::TimerEvent)
		{
			...
			//this->planeWidget->SetNormal(roll, elevation, azimuth);
			...
			//this->guidance_cutplane->SetOrientation(azimuth, elevation, roll);

			/*azimuth = this->adjust_azimuth;
			elevation = this->adjust_elevation;
			roll = this->adjust_roll;*/
			...
			case Cardiac_Windows::Parasternal_1:
				...
				double* orientation;
				orientation = this->imageslice_par_1[this->current_volume_index]->GetOrientation();
				imageslice_par_1[this->current_volume_index]->SetOrientation(angle_adjust);
			...
			if(this->current_cardiac_window != Cardiac_Windows::None)
			{
				...
				matrix->MultiplyPoint(norm, this->normal);
			...
			//guidance_volume->SetOrientation(30, 90, 0);//this->adjust_azimuth, this->adjust_elevation, this->adjust_roll);
		...
		if(event == vtkCommand::KeyPressEvent)
		{
			...
			if(key == "k")
			{
				vtkCamera* cam = this->guid_renderer->GetActiveCamera();

				double *ab;
				ab = cam->GetOrientation();
				cout<<"Current cam orientation: "<<ab[0]<<" "<<ab[1]<<" "<<ab[2]<<endl;
				ab = cam->GetViewUp();
				cout<<"Cam view up: "<<ab[0]<<" "<<ab[1]<<" "<<ab[2]<<endl;

			}

			if(key == "1")
			{
				this->guidance_volume->VisibilityOff();				
			}

			if(key == "2")
			{
				this->guidance_volume->VisibilityOn();
			}

			/*if(key == "a" && this->adjust_azimuth < 180)
			{
				this->adjust_azimuth += 1;
			}

			else if(key == "z" && this->adjust_azimuth > -180)
			{
				this->adjust_azimuth -= 1;
			}

			else if(key == "s" && this->adjust_elevation < 180)
			{
				this->adjust_elevation += 1;
			}

			else if(key == "x" && this->adjust_elevation > -180)
			{
				this->adjust_elevation -= 1;
			}

			else if(key == "d" && this->adjust_roll < 180)
			{
				this->adjust_roll += 1;
			}

			else if(key == "c" && this->adjust_roll > -180)
			{
				this->adjust_roll -= 1;
			}*/

			//cout<<"Azimuth :"<<this->adjust_azimuth<<" Elevation :"<<this->adjust_elevation<<" Roll : "<<this->adjust_roll<<endl;
...
#pragma endregion
	...
	{		
		...
		if(i == 0)
		{
			...
			mc->SetInputConnection(vtkImporter->GetOutputPort());
			mc->ComputeNormalsOn();
			mc->ComputeGradientsOn();
			mc->SetValue(10, 50);  // second value acts as threshold	
	...
	callback->guid_plane_polydata = plane;

	callback->probe = vtkSmartPointer<vtkProbeFilter>::New();
	callback->probe->SetInput(plane);
	callback->probe->SetSource(mc->GetOutput());

	vtkSmartPointer<vtkPolyDataMapper> contourMapper = vtkSmartPointer<vtkPolyDataMapper>::New();
	contourMapper->SetInputConnection(callback->probe->GetOutputPort());
	contourMapper->SetScalarRange(mc->GetOutput()->GetScalarRange());
	...
	//planeWidget->AddObserver(vtkCommand::TimerEvent, callback);
	
	
	//Adding guidance actor
	vtkSmartPointer<vtkRenderer> guid_renderer =
		vtkSmartPointer<vtkRenderer>::New();
	...
	callback->throw_plane_actor->SetMapper(contourMapper);
