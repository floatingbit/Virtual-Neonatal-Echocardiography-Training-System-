...
public:
	...
	vtkImageInteractionCallback() {
			...
			this->subcostal_window_1 = new Range(0, 10, 0, 10, 0, 10, 0, 0, 0, 0, 0, 0);
			this->subcostal_window_2 = new Range(-10, 0, 0, 10, 0, 10, 0, 0, 0, 0, 0, 0);
	...
	{		
			...
			z >= this->subcostal_range->min_z && z <= this->subcostal_range->max_z)
		{
			...
			if(azimuth >= this->subcostal_range->min_azimuth && azimuth <= this->subcostal_range->max_azimuth && 
			elevation >= this->subcostal_range->min_elevation && elevation <= this->subcostal_range->max_elevation && 
			roll >= this->subcostal_range->min_roll && roll <= this->subcostal_range->max_roll)
			{
				cout<<"Subcostal 1 detected"<<endl;
				return Cardiac_Windows::Subcostal_1;
			}

			else if(azimuth >= this->subcostal_range->min_azimuth && azimuth <= this->subcostal_range->max_azimuth && 
			elevation >= this->subcostal_range->min_elevation && elevation <= this->subcostal_range->max_elevation && 
			roll >= this->subcostal_range->min_roll && roll <= this->subcostal_range->max_roll)
			{
				cout<<"Subcostal 2 detected"<<endl;
